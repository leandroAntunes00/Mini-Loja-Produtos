...
export default function ProductCard({ product, onAdd, loading = false }) {
  return (
    <article
      ...
      className="bg-surface dark:bg-surface rounded-lg shadow-sm overflow-hidden hover:-translate-y-1 transition-transform"
    >
      ...
      <div className="p-3">
        ...
        <div className="flex items-center gap-3 text-muted text-sm mt-2">
          ...
          {product.tag ? (
            <div className="px-2 py-0.5 rounded-full bg-primary text-white text-xs">
              {product.tag}
            </div>
          ) : null}
        </div>
