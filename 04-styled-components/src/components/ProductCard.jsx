...
const Meta = styled.div`display:flex;gap:8px;align-items:center;color:${p=>p.theme.muted};font-size:14px`
const Price = styled.div`font-weight:700`
const TagBadge = styled.div`background:${p=>p.theme.accent};color:white;padding:4px 8px;border-radius:999px;font-size:12px`
const Actions = styled.div`margin-top:12px`

export default function ProductCard({product,onAdd,loading=false}){
  return (
    <Card tabIndex={0} aria-labelledby={`title-${product.id}`}>
  <ImageWrap>{loading ? <Skeleton /> : <img src={product.image} alt={product.title} loading="lazy" style={{width:'100%',height:'100%',objectFit:'cover'}} />}</ImageWrap>
      <Content>
        ...
        <Actions><Button variant="solid" onClick={()=>onAdd(product)}>Adicionar</Button></Actions>
      </Content>
    </Card>
  )
}
